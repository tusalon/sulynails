...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '59b4f82a-8909-4033-a76b-2eefe6e64f07'; // ID de Suly.nails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Suly.nails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '53402639';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'sulaysolorzanochung@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Suly.nails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'sulynails';
};
...
console.log('✅ config-negocio.js listo para Suly.nails');
